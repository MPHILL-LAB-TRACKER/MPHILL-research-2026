...
(() => {
 ...
 document.querySelectorAll('[data-carousel]').forEach(carousel=>{
  ...
  let index=0,manual=carousel.dataset.autoplay!=='yes'||reduced(),hover=false,focus=false;const isFacts=carousel.hasAttribute('data-facts');let seen=[];
  if(isFacts){try{seen=JSON.parse(sessionStorage.getItem('ted2-reviewed-facts')||'[]');if(!Array.isArray(seen))seen=[];}catch{seen=[];}const i=slides.findIndex(s=>!seen.includes(s.dataset.factId));index=i<0?0:i;}
  const remember=()=>{if(!isFacts)return;const id=slides[index].dataset.factId;if(slides.every(s=>seen.includes(s.dataset.factId)))seen=[];if(id&&!seen.includes(id))seen.push(id);try{sessionStorage.setItem('ted2-reviewed-facts',JSON.stringify(seen.slice(-100)));}catch{/* Storage may be blocked; in-page rotation still works. */}};
