...
final class Schema {
 ...
 public static function default(array $f):mixed{if(array_key_exists('default',$f))return $f['default'];return in_array($f['type'],['multi','lines','urls','multi-choice'],true)?[]:($f['type']==='checkbox'?false:(in_array($f['type'],['number','year','percent','weight','money','quantity'],true)?null:''));}
 public static function validate(string $c,array $p):array{
  ...
  foreach($schema['fields'] as $f){$k=$f['key'];$v=$p[$k]??self::default($f);$t=$f['type'];ensure(!$f['required']||!($v===null||$v===''||$v===[]),422,$f['label'].' is required.');
   ...
   elseif(in_array($t,['number','year','percent','weight','money','quantity'],true)){if($v===''||$v===null)$v=null;else{ensure(is_int($v)||is_float($v),422,$f['label'].' must be numeric.');$bounds=['number'=>[0,10000],'year'=>[1600,2200],'percent'=>[0,100],'weight'=>[1,100],'money'=>[0,1e12],'quantity'=>[0,1e12]][$t];ensure(is_finite((float)$v)&&$v>=$bounds[0]&&$v<=$bounds[1],422,$f['label'].' is out of range.');if(!in_array($t,['money','quantity'],true))ensure((int)$v==$v,422,'Whole number required.');}}
