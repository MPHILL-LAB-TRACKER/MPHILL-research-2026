"""Conservative, repeatable V2→V3 seed update for an existing installation.

Only fields still equal to the shipped V2 baseline are replaced. Owner edits,
...
def upgrade_content(store, *, apply=False):
    ...
    if apply:
        ...
        backup = directory / f'pre-content-v3-{stamp}-{uuid.uuid4().hex[:8]}.sqlite3'
    ...
    with store.connect(write=apply) as connection:
        for (collection, rid), new in proposed.items():
            ...
            if row is None:
                ...
                if apply:
                    connection.execute('INSERT INTO records VALUES(?,?,?,?,1,?,?)',
                        (collection, rid, json.dumps(new, ensure_ascii=False), new['visibility'], 'content-v3', now()))
                    store.audit(connection, 'server-operator', 'content-v3-create', collection, rid, after=new)
                continue
            old = previous.get((collection, rid))
            ...
            for key, value in new.items():
                if key == 'id' or value == old.get(key) or current.get(key) == value:
                    continue
            ...
            if apply:
                ...
                store.audit(connection, 'server-operator', 'content-v3-update', collection, rid, before=current, after=merged)
    return report
