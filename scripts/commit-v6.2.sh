...
if (($m["version"]??"")!=="6.2.0" || !is_array($m["files"]??null)) exit(1);
...
printf '\nCommit only %s V6.2 release paths. Database, .env, uploads and private backups are excluded.\n' "${#FILES[@]}"
...
git add -- "${FILES[@]}"
if git diff --cached --quiet; then echo 'No V6.2 source changes to commit.'; exit 0; fi
git --no-pager diff --cached --stat
git -c core.pager=cat commit -m 'Upgrade TED2 to V6.2: photographic studio, appearance modes, research pulse and safe caching'
