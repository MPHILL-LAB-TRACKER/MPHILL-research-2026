...
(() => {
    ...
    function pubCard(p) { const url = p.doi ? 'https://doi.org/' + p.doi : p.source_url; return `<article class="publication-card" id="pub-${E(p.id)}"><div class="publication-meta"><span class="record-type ${p.type !== 'peer-reviewed' ? 'limited' : ''}">${E(nice(p.type))}</span><span>${E(p.year)}</span><span>${E(p.venue)}</span></div><h3>${external(url, p.title) || E(p.title)}</h3><p class="authors">${E((p.authors || []).join('; '))}</p><p class="pub-summary">${E(p.summary)}</p><div class="pub-actions">${external(p.source_url, 'Source record')}${p.doi ? external('https://doi.org/' + p.doi, 'DOI') : ''}${p.document_url ? mediaLink(p.document_url, 'Approved PDF') : ''}<button class="copy-button" data-copy="${E(p.id)}">Copy citation</button></div>${p.note ? `<details><summary>Evidence & attribution</summary><p>${E(p.note)}</p></details>` : ''}</article>`; }
    ...
    function customSection(s) {
        ...
        return `<section class="section custom-section layout-${E(s.layout)}" id="section-${E(s.id)}"><div class="container custom-section-inner">${content}${gallery}</div></section>`;
    }
    ...
    ${customHome('after-research')}<section class="section section-cream" id="interactions"><div class="container feature-grid"><div class="feature-typeset"><div><p class="eyebrow">02 / Cell–material interactions</p><h2>Better materials.<br>Closer to biology.</h2></div><div class="feature-rule"><span class="feature-mark">2D / 3D</span><p>Adhesion · proliferation · differentiation</p></div></div><div class="topic-list">${caps.slice(5, 9).map(r => `<article class="topic-row"><h3>${E(r.title)}</h3><p>${E(r.text)}</p></article>`).join('')}</div></div></section>
    ...
    ${publicBioSection('Biography', `${profileContact(p)}<p class="profile-text">${E(p.bio)}</p>${p.work ? `<h3 style="margin-top:24px">Selected work</h3><p class="profile-text">${E(p.work)}</p>` : ''}${p.note ? `<details class="notice"><summary>Evidence note</summary><p>${E(p.note)}</p></details>` : ''}`, 'bio')}
