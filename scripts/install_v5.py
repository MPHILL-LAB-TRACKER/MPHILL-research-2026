...
def main():
    ...
    backup=Path.home()/'TED2-private-backups'/('v5-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-'+uuid.uuid4().hex[:8])
    ...
    print('\nV5 installed. Existing data preserved. No Git commit or push has been made.')
    print('Review and commit: bash scripts/commit-v5.sh\nPush source: git push origin main\nStart backend: .venv/bin/python manage.py serve')
