    ...
    with tempfile.TemporaryDirectory(prefix='ted2-browser-') as temp:
        ...
        try:
            with sync_playwright() as pw:
                ...
                checked('Unavailable named portrait falls back to initials',page.locator('.avatar.large .profile-image').count()==0)
                ...
                page.locator('[name=username]').fill('browser-owner');page.locator('[name=password]').fill('Temporary browser test password 2026!');page.locator('#login-form button').click();page.get_by_role('heading',name='Research, with a clear next step.').wait_for();checked('Owner can sign in and open real administration')
                page.screenshot(path=str(args.output/'admin-dashboard.png'),full_page=False)
                page.goto(base+'/admin#/edit/people/vevangapi-mbatara',wait_until='networkidle');page.locator('#f-bio').fill('BROWSER TEST — biography edit for integration testing only.');page.locator('#record-form [type=submit]').click();page.get_by_text('Saved. Public content updates on the next website load.').wait_for();checked('Biography form saves to the server')
                ...
                page.goto(base+'/admin#/export',wait_until='networkidle');checked('Both public export modes available',page.locator('a[href="/api/export/connected"]').count()==1 and page.locator('a[href="/api/export/snapshot"]').count()==1)
