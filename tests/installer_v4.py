...
import argparse,json,os,shutil,sqlite3,subprocess,sys,sysconfig,tempfile
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--output',type=Path,default=Path('test-results/installer-v4.json'));args=p.parse_args()
    if os.geteuid()==0:raise SystemExit('Run this installer test as a normal non-root account.')
    checks=[]
    def check(name,ok=True):
        if not ok:raise AssertionError(name)
        checks.append(name)
    with tempfile.TemporaryDirectory(prefix='ted2-upgrade-v4-') as temporary:
        ...
        # The required skeleton contains the same byte-for-byte release files. The
        # V3 database below exercises a genuine content transition, not re-seeding.
        ...
        baseline=json.loads((ROOT/'data/updates/v3-baseline.json').read_text())
        ...
        def install():return subprocess.run([sys.executable,str(ROOT/'scripts/install_v4.py'),'--target',str(repo),'--yes','--no-deps'],env=env,capture_output=True,text=True)
        ...
        backups=list((home/'TED2-private-backups').glob('v4-*'));check('Private backup created outside clone',len(backups)==1 and (backups[0]/'ted2.sqlite3').is_file());check('Backup folder private',backups[0].stat().st_mode & 0o777==0o700)
        ...
        check('All V4 backend and publisher code committed','backend/publisher.py' in tracked and 'web/assets/admin.js' in tracked);check('No remote needed or changed',git('remote').stdout=='')
    ...
    report={'checks_passed':len(checks),'checks':checks,'mode':'Disposable existing Git clones and V3 SQLite content; actual installer and source commit subprocesses; no pip/network/remote GitHub actions.'};args.output.parent.mkdir(parents=True,exist_ok=True);args.output.write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
