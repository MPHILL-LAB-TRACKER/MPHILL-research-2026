...
(() => {
    ...
    function shell() {
        ...
        const sections = personal ? [
            ...
            ['Website content', [['records/people', 'Researchers & collaborators'], ['records/publications', 'Publications'], ['records/collaborations', 'Collaborations'], ['records/funders', 'Funders'], ['records/capabilities', 'Homepage research'], ['records/settings', 'Website settings'], ['records/sources', 'Sources & credits']]],
    ...
    function fieldHTML(f, p, readOnly, isNew) {
        ...
        const full = ['textarea', 'lines', 'urls', 'image', 'document'].includes(f.type);
        const label = E(f.label) + (f.public ? '' : ` <small class="private-label">Always private</small>`);
        let control;
        ...
        else if (['image', 'document'].includes(f.type))
            control = `<div class="upload-box"><input type="hidden" id="${id}" name="${E(f.key)}" value="${E(v || '')}"><div id="preview-${E(f.key)}">${v ? `${f.type === 'image' ? `<img class="upload-preview" src="/media/${E(v)}" alt="Current portrait">` : ''}<a href="/media/${E(v)}" target="_blank" rel="noopener">Open current ${f.type === 'image' ? 'portrait' : 'PDF'} ↗</a>` : '<span class="help">No file attached.</span>'}</div>${!readOnly ? `${isNew ? '<p class="help">Save this record first, then upload its file.</p>' : `<input type="file" data-file="${E(f.key)}" accept="${f.type === 'image' ? 'image/jpeg,image/png,image/webp' : 'application/pdf'}" aria-label="Choose ${f.type === 'image' ? 'portrait' : 'PDF'}"><button type="button" class="button secondary" data-upload="${E(f.key)}">Upload ${f.type === 'image' ? 'portrait' : 'PDF'}</button>${v ? `<button type="button" class="text-button" data-remove-file="${E(f.key)}">Detach file</button>` : ''}`}` : ''}<span class="help">${f.type === 'image' ? 'JPEG, PNG or WebP up to 5 MB. Set portrait permission to Approved before making the local image public.' : 'PDF up to 20 MB. Manuscript, milestone and update attachments always remain private. Uploaded PDFs are not antivirus-scanned.'}</span></div>`;
        else {
    ...
    async function edit(collection, rid) {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                dirty = false;
                await refresh();
                notify(saved.visibility === 'public' ? 'Saved. Public content updates on the next website load.' : 'Private record saved.');
                if (isNew)
    ...
    async function render() {
        ...
        if (!admin() && ['users', 'audit', 'export', 'board'].includes(route)) {
