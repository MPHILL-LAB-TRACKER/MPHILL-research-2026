...
(() => {
    ...
    const link = (path, label, cls = '') => `<a class="${E(cls)}" href="#/${E(path)}">${E(label)}</a>`;
    ...
    function home() {
        ...
        const roster = group => rows('people').filter(p => p.group === group).map(p => `<a class="roster-link" href="#/profiles/${E(p.id)}">${avatar(p, 'small')}<span>${E(p.name)}</span><span class="arrow" aria-hidden="true">↗</span></a>`).join('');
    ...
    <nav class="section-nav" aria-label="Homepage sections"><div class="container"><span>Explore</span><a href="#/?section=research">Research capabilities</a><a href="#/?section=interactions">Cell–material interactions</a><a href="#/?section=scaffolds">Scaffolds & applications</a><a href="#/?section=people">Our people</a></div></nav>
    ...
    function profile(id) {
        id = id === 'mbotarai-vevangapi' ? 'vevangapi-mbatara' : id;
        const p = one('people', id);
        if (!p)
            return notFound();
        const pubs = rows('publications').filter(x => (x.people || []).includes(id)), projects = rows('projects').filter(x => x.lead_id === id || (x.people || []).includes(id)), manuscripts = rows('manuscripts').filter(x => (x.people || []).includes(id)), collabs = rows('collaborations').filter(x => (x.people || []).includes(id)), updates = rows('updates').filter(x => x.researcher_id === id);
    ...
    <section class="container page-body directory-layout">${side()}<div><nav class="profile-subnav" aria-label="Profile sections">${[['bio', 'Biography'], ['publications', 'Publications'], ['research', 'Active research'], ['pipeline', 'Manuscripts'], ['activity', 'Activity']].map(([a, t]) => link('profiles/' + id + '?section=' + a, t)).join('')}</nav>
    ...
    ${publicBioSection('Research activity', updates.length ? `<div class="record-grid">${updates.sort((a, b) => (b.date || '').localeCompare(a.date || '')).map(updateCard).join('')}</div>` : empty('No public activity posts yet.', 'Approved updates will appear here as the researcher’s work progresses.'), 'activity')}
    </div></section>`;
    }
    ...
    function render() {
        ...
        const route = bits[0] || '';
        nav.innerHTML = [['', 'Home'], ['researchers', 'Researchers'], ['publications', 'Publications'], ['research', 'Research'], ['pipeline', 'Pipeline'], ['collaborators', 'Collaborations']].map(([u, t]) => `<a href="#/${u}"${route === u ? ' aria-current="page"' : ''}>${t}</a>`).join('');
        nav.classList.remove('open');
        document.getElementById('menu-toggle').setAttribute('aria-expanded', 'false');
        main.innerHTML = route === '' ? home() : route === 'researchers' ? researchers() : route === 'publications' ? publications() : ['profiles', 'profile'].includes(route) ? profile(bits[1]) : route === 'research' ? research(bits[1]) : route === 'pipeline' ? pipeline() : ['collaborations', 'collaborators', 'projects'].includes(route) ? collaborations(bits[1]) : route === 'funders' ? funders() : route === 'sources' ? sources() : route === 'contact' ? contact() : route === 'access' ? access() : notFound();
        wireImages();
        bindSearch(route);
