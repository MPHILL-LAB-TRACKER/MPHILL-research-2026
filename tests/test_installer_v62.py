"""Non-destructive V6.1-to-V6.2 upgrade and allowlisted commit integration test.

Uses a copied V6.1 release, fake private data and a temporary local Git repository.
No Git fetch/push or network access. Set TED2_V61_FIXTURE to an extracted V6 package.
...
V61=Path(os.environ.get('TED2_V61_FIXTURE',str(ROOT.parent/'TED2-Research-Workspace-v6.1')))
if not V61.joinpath('php/Application.php').is_file():
    raise SystemExit('Set TED2_V61_FIXTURE to the extracted original V6.1 package for this integration test.')
...
with tempfile.TemporaryDirectory(prefix='ted2-installer-test-') as td:
    ...
    home=tmp/'home';home.mkdir();repo=home/'Research workspace'/'MPHILL-research-2026';repo.parent.mkdir(parents=True)
    shutil.copytree(V61,repo,ignore=shutil.ignore_patterns('var','.git','.venv','__pycache__','test-results','.pytest_cache'))
    # Current V6 main still contains this V6.1 release marker.
    (repo/'VERSION').write_text('6.1.0\n')
...
"""
    ...
    prompted=run(['php',str(ROOT/'installer/upgrade.php'),'--dry-run'],input=str(repo)+'\n');check('Dry run passed' in prompted.stdout,'Directory-neutral prompt accepts an installation path with spaces')
    dry=run(installer+['--dry-run']);check('Dry run passed' in dry.stdout,'V6 upgrade dry run succeeds')
    check(not (repo/'php/Upgrade62.php').exists(),'Dry run does not install V6.2 source')
    if root_user:
    ...
    result=run(installer+['--yes']);check('V6.2 installed' in result.stdout,'Complete V6.1-to-V6.2 in-place installation')
    check((repo/'VERSION').read_text()=='6.2.0\n','V6.1 version marker is safely updated')
    ...
    with sqlite3.connect(var/'ted2.sqlite3') as db:
        ...
        check(db.execute("SELECT value FROM v6_meta WHERE key='migration-6.2'").fetchone() is not None,'V6.2 additive migration completed')
        ...
        check([x for x in theme['home_blocks'] if x not in ['pulse','quotes']]==['alerts','research','people'],'Owner homepage section order preserved while new reading blocks are added')
        check(theme['notice_seconds']==5,'Existing five-second notice interval is preserved')
        check(theme['appearance']=='system','Missing appearance preference receives system default')
        check(db.execute("SELECT COUNT(*) FROM records WHERE collection='science_quotes' AND visibility='public'").fetchone()[0]==0,'Seed quotations are private pending administrator review')
    ...
    backups=list((home/'TED2-private-backups').glob('v6.2-*'))
    ...
    p=run(['bash','scripts/commit-v6.2.sh'],input='COMMIT\n',checkcode=False)
    ...
    run(['git','add','php/Upgrade62.php','README.md'])
    ...
    check('php/Upgrade62.php' in changed and 'release-manifest.json' in changed,'Commit includes new backend and integrity manifest')
